...
NCP::PluginFactory::query( const NC::FactImpl::ScatterRequest& cfg ) const
{
  ...
  if ( ! cfg.get_sans() )
    return NC::Priority::Unable;//sans disabled, do nothing.

  //This plugin handles SANS by ad-hoc addition of physics in a single-phase
  //material. Thus, despite what one might think, we can actually NOT handle
...
NCP::PluginFactory::produce( const NC::FactImpl::ScatterRequest& cfg ) const
{
  ...
  //Now we just need to combine this with all the other physics. For good
  //measure, we put sans=0 for those, although it is unlikely that there will be
  //any, given that we do not handle multiphase requests:
  auto sc_std = globalCreateScatter( cfg.modified("sans=0") );
